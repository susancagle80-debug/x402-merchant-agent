...
exports.createSolanaKitSigner = createSolanaKitSigner;
 ...
 * ```
 */
function createSolanaKitSigner(client, { walletId, address, authorizationContext, caip2 }) {
    ...
    return {
        ...
        async signMessages(messages) {
            return await Promise.all(messages.map(async (message) => {
                ...
                const signatureBytes = base_1.base64.decode(response.signature);
                return {
        ...
        async signTransactions(transactions) {
            return await Promise.all(transactions.map(async (transaction) => {
                ...
                const transactionBase64 = (0, kit_1.getBase64EncodedWireTransaction)(transaction);
                const response = await client
                    .wallets()
                    .solana()
                ...
                const signedTransactionBytes = base_1.base64.decode(response.signed_transaction);
                const decoder = (0, kit_1.getTransactionDecoder)();
        ...
        async signAndSendTransactions(transactions) {
            if (!caip2) {
                throw new Error('CAIP-2 is required for sending transactions. Please provide one in the createSolanaKitSigner input.');
            }
            return await Promise.all(transactions.map(async (transaction) => {
                // Convert compiled transaction to wire format base64
                const transactionBase64 = (0, kit_1.getBase64EncodedWireTransaction)(transaction);
                ...
                return base_1.base58.decode(response.hash);
            }));
        },
    };
}
//# sourceMappingURL=solana-kit.js.map