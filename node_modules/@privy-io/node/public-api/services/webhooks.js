...
class PrivyWebhooksService {
    constructor(webhooksResource, webhookSigningSecret) {
     ...
     */
    verify({ payload, headers, svix, signing_secret }) {
        const secret = signing_secret ?? this.webhookSigningSecret;
        if (!secret) {
            throw new InvalidWebhookError('Webhook signing secret is required. Pass it to PrivyClient constructor or to verify().');
        }
        ...
        if (headers) {
            ...
            throw new InvalidWebhookError('Webhook headers are required (svix-id, svix-timestamp, svix-signature).');
        }
        try {
